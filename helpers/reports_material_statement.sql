...
select * FROM accounting.warehouse_total_routing;
	...
	with main_warehouse as (
		...
		where financing = 'budget'
		and (54321 is null or 54321 = credit) -- CHANGE IT TO DEBIT LATER!!!!!!!!!
	),
	movements as (
		select
			name_id,
		...
		WHERE financing = 'budget'
		and (54321 is null or 54321 = debit)
	),
	all_warehouses as (
	...
	warehouse_exports as (
		...
		where financing = 'budget'
		and (54321 is null or 54321 = credit)
		group by
			name_id, 
			unit_price, 
			storage_location_id,
